...
%% This paper introduces \lambda_c, the first time-complexity type system with recursive types. \lambda_c lets functional programmers specify time-complexity bounds of functions, and the type system checks and guarantees such bounds. \lambda_c is a milestone towards the "great synthesis" of complexity analysis and programming languages, for it supports recursive types, universal polymorphism and type-level operators, the combined power of which enables generic algebraic data types, the watershed of practical usefulness. Potential beneficiaries of \lambda_c includes everyday functional programmers and computer theorists, using it both as a practical bug-preventing tool and as a formal logic to prove algorithm complexities.

%% Soundness of \lambda_c is proved using the Logical Step-indexed Logical Relation. We formalize our design and proof in the Coq proof assistant (in progress). Key technical contributions include:
%% (*) Realization that the number of unfold-fold reductions is the only metric needed in counting a program's asymptotic complexity.
%% (*) Introduction of structured "size", a generalization of the "input size" in complexity analysis.
%% (*) Introduction of "hide type".
%% (*) Introduction of structured "width", a generalization of the "constant factor" in asymptotic complexity, especially for higher-order functions.
%% (*) Strengths and weaknesses of different language encoding schemes in Coq.
