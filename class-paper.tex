...
Since this system is strictly more restrictive than System F, Thoerem \ref{thm-safety} is straight-forward to prove, and we will focus on the proof of Theorem \ref{thm-boundedness} (in its more general form). The proof is done by Logical Step-indexed Logical Relation~\cite{dreyer2009logical}.

Logical Relations (or Logical Predicates for unary relations as in this case) are essentially interpretations of types. By interpreting a type into the set of ``good expressions'' that have the right type and property, Logical Relations can be used to prove that any well-typed expression has the wanted property. Following the common practice
