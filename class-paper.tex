...
Figure \ref{typing} gives representative typing rules. Typing judgments have the form $\Gamma\typing{e}{\tau}{c}{s}$. $c$ stands for the time bound of $e$ to execute to a value. $s$ stands for the size bound of the final value. The typing rules formulate our intuition of how to count the reduction steps. In the typing context $\Gamma$ (defined in Figure \ref{syntax}), we reuse term variables $x$ to mean both a value and its size (conflating term variable and size variable), so the typing context becomes dependent (entries in it can depend on entries before them), and type variables become interleaved with term variables. But still our type system is not dependent-typed. In each typing entry $x:(\tau,\optional{s})$ of $\Gamma$ there is an optional size $\optional{s}$. The reason for it is that not all term variables stand for function arguments, for whose size there is no constraint. Some term variables are aliases of (parts of) other variables, such as the variables introduced in pattern-match clauses. This option size $\optional{s}$ is for recording this aliasing relation, as can be seen in the {\sc MatchPair} rule.
