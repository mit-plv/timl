...
{\Gamma\vdash w_1 : \tau_1 \\ \Gamma\vdash w_2 : \tau_2 \\ \symclp(\tau_1\times\tau_2)\not\equal\symunit}
{\Gamma\vdash (w_1,w_2) : \tau_1\times\tau_2} 

\inferrule* [Right=WPair']
{\symclp(\tau_1\times\tau_2)\equal\symunit}
{\Gamma\vdash \symtt : \tau_1\times\tau_2} 

\end{mathpar}
}
\caption{\label{wtyping}Typing rules for widths}
\end{figure}

The most interesting rules in Figure \ref{wtyping} are {\sc WPair} and {\sc WPair'}. They split the typing for product type into two cases, depending on the result of $\symclp$. $\symclp$ stands for ``collapse'', whose definition is given in Figure \ref{width-aux}. It collapses the part of a type that does not contain arrows into $\symunit$. Since types can have applications of type-level operators, $\symclp$ must evaluate a type into a normal form firstly. This evaluation is terminating and is defined by recursion on type and its kind. This is one of the reasons why we put kinds in the syntactic class of type.
