...
(*fun test_cps () =
  let
      val tctx = [CArrow (CTypeInt, T1, CArrow (CTypeInt, T1, CTypeInt)), CArrow (CTypeInt, T1, CTypeInt), CTypeInt, CArrow (CTypeInt, T1, CTypeInt), CArrow (CTypeInt, T1, CTypeInt), CTypeInt, CArrow (CTypeInt, T0, CTypeUnit)]
      val tctx = List.map (CPS.transform_type) tctx
      val ctx = ([], tctx)
      val d1 = TyVar (ctx, EVar 1, CArrow (CTypeInt, T1, CTypeInt), T0)
      val d2 = TyVar (ctx, EVar 2, CTypeInt, T0)
      val d3 = TyApp (as_TyApp d1 d2, d1, d2)
      val d4 = TyVar (ctx, EVar 0, CArrow (CTypeInt, T1, CArrow (CTypeInt, T1, CTypeInt)), T0)
      val d5 = TyApp (as_TyApp d4 d3, d4, d3)
      val d6 = TyVar (ctx, EVar 4, CArrow (CTypeInt, T1, CTypeInt), T0)
      val d7 = TyVar (ctx, EVar 5, CTypeInt, T0)
      val d8 = TyApp (as_TyApp d6 d7, d6, d7)
      val d9 = TyVar (ctx, EVar 3, CArrow (CTypeInt, T1, CTypeInt), T0)
...
fun test_absc_appc () =
  let
      ...
      val () = check_typing d2
      val e3 = EAppC (e2, CTypeInt)
      val ct3 = ct2
      val i3 = T0
      val t3 = CSum (CTypeUnit, CTypeInt)
      val d3 = TyAppC ((ct3, e3, t3, i3), d2, KdConst (fst ct3, CTypeInt, KType))
      val () = check_typing d3
      val cps_ty = CPSPass.cps_deriv d3
      val () = println $ str_expr $ #2 (extract_judge_typing cps_ty)
      val () = check_typing cps_ty
      val wrap_ty = WrapAbsPass.wrap_abs_deriv cps_ty
      val () = println $ str_expr $ #2 (extract_judge_typing wrap_ty)
      val () = check_typing wrap_ty
      val clo_ty = CloConvPass.clo_conv_deriv wrap_ty
...
fun test_abs_app () =
  let
      ...
      val ct1 = ([], [])
      val i1 = T0
      val t1 = CArrow (CTypeInt, T0, CTypeInt)
      val d1 = TyAbs ((ct1, e1, t1, i1), KdConst ([], CTypeInt, KType), TyVar (([], [CTypeInt]), EVar 0, CTypeInt, T0))
      val () = check_typing d1
      val d2 = TyApp (as_TyApp d1 (TyConst (([], []), EConst (ECInt 2), CTypeInt, T0)), d1, TyConst (([], []), EConst (ECInt 2), CTypeInt, T0))
      val () = check_typing d2
      val cps_ty = CPSPass.cps_deriv d2
      val () = println $ str_expr $ #2 (extract_judge_typing cps_ty)
      val () = check_typing cps_ty
      val wrap_ty = WrapAbsPass.wrap_abs_deriv cps_ty
      val () = println $ str_expr $ #2 (extract_judge_typing wrap_ty)
...
fun test_currying () =
  let
      ...
      val ct1 = ([], [])
      val i1 = T0
      val t1 = CArrow (CTypeInt, T0, CArrow (CTypeInt, Tadd (T0, T0), CProd (CTypeInt, CTypeInt)))
      val d1 = TyAbs ((ct1, e1, t1, i1), KdConst ([], CTypeInt, KType), TyAbs ((([], [CTypeInt]), EAbs (EPair (EVar 1, EVar 0)), CArrow (CTypeInt, Tadd (T0, T0), CProd (CTypeInt, CTypeInt)), T0), KdConst ([], CTypeInt, KType), TyPair ((([], [CTypeInt, CTypeInt]), EPair (EVar 1, EVar 0), CProd (CTypeInt, CTypeInt), Tadd (T0, T0)), TyVar (([], [CTypeInt, CTypeInt]), EVar 1, CTypeInt, T0), TyVar (([], [CTypeInt, CTypeInt]), EVar 0, CTypeInt, T0))))
      val () = check_typing d1
      val d2 = TyApp (as_TyApp d1 (TyConst (([], []), EConst (ECInt 2), CTypeInt, T0)), d1, TyConst (([], []), EConst (ECInt 2), CTypeInt, T0))
      val () = check_typing d2
      val d3 = TyApp (as_TyApp d2 (TyConst (([], []), EConst (ECInt 6), CTypeInt, T0)), d2, TyConst (([], []), EConst (ECInt 6), CTypeInt, T0))
      val () = check_typing d3
      val cps_ty = CPSPass.cps_deriv d3
      val () = println $ str_expr $ #2 (extract_judge_typing cps_ty)
      val () = check_typing cps_ty
...
fun test_concat () =
  let
      ...
      val i37 = i36
      val d37 = TyFold ((ct37, e37, t37, i37), KdApp ((fst ct37, t37, KType), KdApp ((fst ct37, CApp (list_dec, CVar 4), KArrow (KNat, KType)), shift0_ctx_kd (fst ct37) list_kd, KdVar (fst ct37, CVar 4, KType)), KdBinOp ((fst ct37, CBinOp (CBNatAdd, CVar 3, CVar 2), KNat), KdVar (fst ct35, CVar 3, KNat), KdVar (fst ct37, CVar 2, KNat))), d36)
      ...
      val i46 = T0
      val d46 = TyAbs ((ct46, e46, t46, i46), KdBinOp ((fst ct46, CProd (CApps list_dec [CVar 2, CVar 1], CApps list_dec [CVar 2, CVar 0]), KType), KdApp ((fst ct46, CApps list_dec [CVar 2, CVar 1], KType), KdApp ((fst ct46, CApp (list_dec, CVar 2), KArrow (KNat, KType)), shift0_ctx_kd (fst ct46) list_kd, KdVar (fst ct46, CVar 2, KType)), KdVar (fst ct46, CVar 1, KNat)), KdApp ((fst ct46, CApps list_dec [CVar 2, CVar 0], KType), KdApp ((fst ct46, CApp (list_dec, CVar 2), KArrow (KNat, KType)), shift0_ctx_kd (fst ct46) list_kd, KdVar (fst ct46, CVar 2, KType)), KdVar (fst ct46, CVar 0, KNat))), d45)
      ...
      val cps_ty = CPSPass.cps_deriv d50
      val () = println $ str_expr $ #2 (extract_judge_typing cps_ty)
      val () = check_typing cps_ty
      val wrap_ty = WrapAbsPass.wrap_abs_deriv cps_ty
      val () = println $ str_expr $ #2 (extract_judge_typing wrap_ty)
