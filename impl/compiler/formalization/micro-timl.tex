...
\begin{array}{rcl}
	...
	\tau & ::= & \tau \xrightarrow[i \mid i]{i} \tau \mid \cdots
\end{array}
\]
...
\begin{array}{l}
	...
	\infer[\textsc{KdArrow}]{\mathcal{K} \vdash \tau_1 \xrightarrow[m \mid q]{i} \tau_2 :: \star(q + 1)}{\mathcal{K} \vdash \tau_1 :: \star(i_1) & \mathcal{K} \vdash \tau_2 :: \star(i_2) & \mathcal{K} \vdash i :: \mathbf{TimeFun}_0 & \mathcal{K} \vdash m :: \mathbf{Nat} & \mathcal{K} \vdash q :: \mathbf{Nat}} \\
...
\begin{array}{l}
	...
	\infer[\textsc{TyAbs}]{\mathcal{C} \vdash \lambda x:\tau. e : \tau \xrightarrow[m \mid |FV(e) \setminus \{x \}|]{i} \tau_1 \triangleright 0 \mid |FV(e)\setminus \{ x \}| +1}{\mathcal{C}.1 \vdash \tau :: \star & \mathcal{C},x:\tau \vdash e : \tau_1 \triangleright i \mid m} \\
	\infer[\textsc{TyApp}]{\mathcal{C} \vdash e_1~e_2 : \tau_2 \triangleright i_1 + i_2 + 1 + i \mid m_1 + m_2 + m}{\mathcal{C} \vdash e_1 : \tau_1 \xrightarrow[m \mid q]{i} \tau_2 \triangleright i_1 \mid m_1 & \mathcal{C} \vdash e_2 : \tau_1 \triangleright i_2 \mid m_2} \\
	\infer[\textsc{TyPair}]{\mathcal{C} \vdash (e_1,e_2) : \tau_1 \times \tau_2 \triangleright i_1 + i_2 \mid m_1 + m_2 + 2}{\mathcal{C} \vdash e_1 : \tau_1 \triangleright i_1 \mid m_1 & \mathcal{C} \vdash e_2 : \tau_2 \triangleright i_2 \mid m_2} \\
	...
	\infer[\textsc{TyAbsC}]{\mathcal{C} \vdash \Lambda \alpha :: \kappa. e : \forall \alpha :: \kappa. \tau \triangleright 0 \mid m}{\mathcal{C}.1 \vdash \mathrm{wf}~\kappa & \vdash \mathrm{value}~e & \mathcal{C},\alpha :: \kappa \vdash e : \tau \triangleright 0 \mid m & \mathcal{C}.1,\alpha :: \kappa \vdash \tau :: \star(i) & m,i~\mathrm{do~not~contain}~\alpha} \\
	...
	\infer[\textsc{TyNew}]{\mathcal{C} \vdash \mathbf{new}~e_1~e_2 : \mathbf{arr}~\tau~j \triangleright i_1 + i_2 \mid m_1 + m_2 + j}{\mathcal{C} \vdash e_1 : \tau \triangleright i_1 \mid m_1 & \mathcal{C}.1 \vdash \tau :: \star(s) & \mathcal{C} \vdash e_2 : \mathbf{nat}~j \triangleright i_2 \mid m_2} \\
