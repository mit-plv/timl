...
fun transform_type t =
  case t of
    ...
    | CRec (name, k, c) => CRec (name, k, transform_type c)
...
fun transform_kinding kd =
  case kd of
    ...
    | KdRec ((kctx, CRec (name, k, c), _), wk, kd) =>
      let
          val kd = transform_kinding kd
      in
          KdRec (as_KdRec name wk kd, wk, kd)
      end
...
fun transform_tyeq te =
  case te of
    ...
    | TyEqRec (j, ke, te) =>
      let
          val (name1, _, _) = extract_c_rec (#2 j)
          val (name2, _, _) = extract_c_rec (#3 j)
          val te = transform_tyeq te
      in
          TyEqRec (as_TyEqRec name1 name2 ke te, ke, te)
      end
    | TyEqRef (j, te) =>
      let
          val te = transform_tyeq te
...
fun wrap_lambda_deriv ty = ExprHelper.transform_typing (ty, ((), ()))
end
