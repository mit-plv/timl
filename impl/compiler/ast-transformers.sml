...
functor CstrGenericTransformer(
    Action:
    sig
        type down
        type up
...
functor ExprGenericTransformer(
    ...
    sig
        ...
        val add_kind : kind option * down -> down
        val add_type : cstr option * tdown -> tdown

        val transform_cstr : cstr * kdown -> cstr * up

        val transformer_expr : (expr * down -> expr * up) -> expr * down -> (expr * up) option
    end) =
struct
open List
...
fun default_transform_expr (e, down as (kdown, tdown)) =
    case e of
      ...
      | ECase (e, e1, e2) =>
        let
            val (e, up1) = transform_expr (e, down)
            val (e1, up2) = transform_expr (e1, (kdown, Action.add_type (NONE, tdown)))
            val (e2, up3) = transform_expr (e2, (kdown, Action.add_type (NONE, tdown)))
        in
            (ECase (e, e1, e2), combine [up1, up2, up3])
        end
      | EAbs e =>
        let
            val (e, up1) = transform_expr (e, (kdown, Action.add_type (NONE, tdown)))
        in
            (EAbs e, combine [up1])
        end
      | ERec e =>
      ...
      | EAppC (e, c) =>
        let
            val (e, up1) = transform_expr (e, down)
            val (c, up2) = Action.transform_cstr (c, kdown)
        in
            (EAppC (e, c), combine [up1, up2])
        end
      | EPack (c, e) =>
        let
            val (c, up1) = Action.transform_cstr (c, kdown)
      ...
      | EUnpack (e1, e2) =>
        let
            val (e1, up1) = transform_expr (e1, down)
            val (e2, up2) = transform_expr (e2, let val (kdown, tdown) = Action.add_kind (NONE, down) in (kdown, Action.add_type (NONE, tdown)) end)
        in
      ...
      | ELet (e1, e2) =>
        let
            val (e1, up1) = transform_expr (e1, down)
            val (e2, up2) = transform_expr (e2, (kdown, Action.add_type (NONE, tdown)))
        in
...
struct
structure Transformer = CstrGenericTransformer(
    struct
...
functor ExprGenericOnlyDownTransformer(
    Action:
    sig
        type kdown
        type tdown
        ...
        val transform_cstr : cstr * kdown -> cstr

        val transformer_expr : (expr * down -> expr) -> expr * down -> expr option
    end) =
struct
structure Transformer = ExprGenericTransformer(
    struct
    ...
    val transform_cstr = (fn c => (c, ())) o Action.transform_cstr
...
structure ExprHelper = ExprGenericOnlyDownTransformer(
    struct
    type kdown = int * int
    type tdown = unit
    type down = kdown * tdown
...
structure CstrHelper = CstrGenericTransformer(
    struct
    type down = int
    type up = int list
