...
(defvar sml-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "\C-c\C-s" 'sml-insert-form)
    ...
    (define-key map "\C-c\C-m" 'sml-prog-proc-switch-to)
    map)
...
(defconst sml-non-nested-of-starter-re
  (sml-syms-re '("datatype" "abstype" "absidx" "exception"))
...
(defconst sml-pipeheads
  '("|" "of" "fun" "fn" "and" "handle" "datatype" "abstype" "absidx"
    "(" "{" "[")
...
Assumes point is right before the | symbol."
  (save-excursion
    ...
    (let ((after-type-def
           '("|" "of" "in" "datatype" "and" "exception" "abstype" "absidx" "infix"
...
(defun sml-prog-proc-switch-to ()
  ...
  (interactive)
  (my-pop-to-buffer (sml-prog-proc-buffer)))
...
to the buffer where the process is running."
  ...
  (let ((proc (sml-prog-proc-proc)))
    ...
    (when and-go (my-pop-to-buffer (process-buffer proc)))))
  ...
  (let* ((pname (file-name-nondirectory cmd))
    ...
    (save-current-buffer
      (let ((exec-path (if (and (file-name-directory cmd)
        ...
        (my-pop-to-buffer (apply 'make-comint pname cmd file args)))
...
(defvar inferior-sml-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "\C-c\C-m" 'sml-run)
           ...
           (save-excursion
             ...
             (let ((sym (sml-find-matching-starter sml-pipeheads
               ...
               (cond
                ...
                ;;((member sym '("abstype" "absidx" "datatype")) "")
                (t nil))))))
      (when text
        (save-excursion
          (goto-char (1- acp))
...
(defun sml-beginning-of-defun ()
  (let ((sym (sml-find-matching-starter sml-starters-syms)))
    (if (member sym '("fun" "and" "functor" "signature" "structure"
		      "abstraction" "datatype" "abstype" "absidx"))
    ...
    (if (null (car symdata))
      ...
      (with-current-buffer (find-file-noselect (sml-defuse-file))
        ...
        (let ((line (string-to-number (match-string 3)))
          ...
          (my-pop-to-buffer (find-file-noselect (match-string 2)))
          (goto-char (point-min))
          (forward-line (1- line))
          (forward-char (1- char)))))))
...
;; 
;; 	* packages/sml-mode/sml-mode.el (sml-mode-map): Add back C-c C-m binding.
